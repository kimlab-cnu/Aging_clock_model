...
## method 별로 20s-1부터 50s-6까지 데이터를 분리했음
...
## method1에 대해서 stand1~6까지 y8, y6, y8, y6, y8, y6의 area 값을 구함
...
# stand3 : YVYVADVAAK -> y8 (2번째 transition)
# stand4 : YVYVADVAAK -> y6 (2번째 transition)
# stand5 : YVYVADVAAK -> y8 (3번째 transition)
# stand6 : YVYVADVAAK -> y6 (3번째 transition)
...
write.csv(final_df, file="../output/Transition_PAR_All (IS 포함)_250527.csv", row.names=F)
# Internal Standard를 위에서 분리하지 않아서 Internal Standard의 area 값도 나눠져 있음
# 근데 우리는 실제 분석을 할 때 peptide area 값을 보정해야 되기 때문에 분리가 필요함

final_IS <- final_df %>% filter(grepl("^Peptide", Protein))
final_Sample <- final_df %>% filter(!grepl("^Peptide", Protein))  # Peptide들만 따로 뽑아서

write.csv(final_Sample, file="../output/Transition_PAR_Results_250605.csv", row.names=F)

#### CV 값을 구하기 위해서 stand1부터 6까지 뽑음
## standard 값 저장하기
...
# method별로 stand1~6까지 IS의 area 값을 구함함
